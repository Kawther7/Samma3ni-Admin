...
export const createUser = async (user, dispatch) => {
	...
	try {
		const { data } = await axiosInstance.post("http://www.localhost:8080/api/users/signup", user);
...
export const getAllUsers = async (dispatch) => {
	...
	try {
		const { data } = await axiosInstance.get("http://localhost:8080/api/users");
...
export const updateUser = async (id, user, dispatch) => {
	...
	try {
		const { data } = await axiosInstance.put(`http://localhost:8080/api/users/${id}`, user);
...
export const deleteUser = async (id, dispatch) => {
	...
	try {
		const { data } = await axiosInstance.delete(`http://www.localhost:8080/api/users/${id}`);
